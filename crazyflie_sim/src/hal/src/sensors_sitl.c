...
void sensorsSimInit(void)
{
  ...
  // SITL sensor streams come from the simulator and do not require the same
  // startup bias-finding gate as the physical IMU path. On newer Linux/Gazebo
  // stacks this gate can stall the simulated firmware startup and prevent
  // cflib from completing the link handshake, so start the simulated sensors
  // in a calibrated state with zero gyro bias.
  gyroBias.x = 0.0f;
  gyroBias.y = 0.0f;
  gyroBias.z = 0.0f;
  gyroBiasFound = true;
