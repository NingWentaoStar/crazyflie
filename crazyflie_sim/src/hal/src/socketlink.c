...
static void socketlinkTask(void *param)
{
  ...
  while(1)
  {
    // Use a finite poll timeout on Linux/POSIX SITL. On Ubuntu 24.04 the
    // FreeRTOS POSIX tasks can otherwise end up stuck in an indefinite wait
    // and stop forwarding sensor / CRTP traffic to the firmware.
    int pollResult = poll(&fds[0], (sizeof(fds[0]) / sizeof(fds[0])), 10);
    if (pollResult < 0) {
      if (errno == EINTR) {
        vTaskDelay(M2T(1));
        continue;
